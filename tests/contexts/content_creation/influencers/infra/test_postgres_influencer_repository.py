...
class TestPostgresInfluencerRepository(InfluencerModuleIntegrationTestConfig):
    def test_should_save_a_influencer(self) -> None:
        self.postgres_influencer_repository.save(self.influencer)

        saved_influencer = self.postgres_influencer_repository.search(
            self.influencer.id
        )
    ...
    def test_should_find_an_existing_influencer(self) -> None:
        ...
        searched_influencer = self.postgres_influencer_repository.search(
            self.influencer.id
        )
