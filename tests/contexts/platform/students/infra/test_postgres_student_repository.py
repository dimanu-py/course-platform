...
class TestPostgresStudentRepository(StudentModuleIntegrationTestConfig):
    def test_should_save_a_student(self) -> None:
        student = StudentMother.create()

        self.postgres_student_repository.save(student)

        saved_student = self.postgres_student_repository.search(student.id)
    ...
    def test_should_find_an_existing_student(self) -> None:
        ...
        searched_student = self.postgres_student_repository.search(student.id)
